"""
live_transcriber.py  —  Production-grade dual-stream live transcription
Supports:  English (faster-whisper / small)
           Hinglish (openai/whisper-large-v3  via HuggingFace pipeline)

Key improvements over previous version
───────────────────────────────────────
HINGLISH
  • Model upgrade  : openai/whisper-large-v3  (state-of-the-art for Hindi/
                     Hinglish code-switching; replaces the under-powered Swift
                     model that caused ~88% transcription error rate)
  • Language tag   : "hindi" is retained BUT initial_prompt is injected so
                     the model knows to output Romanised Hindi mixed with
                     English (the standard Hinglish expectation).
  • Decode params  : repetition_penalty 1.5→1.15, no_repeat_ngram_size 5→3,
                     num_beams 1→4  — all tuned for code-switched speech.
  • Temperature    : cascade [0.0, 0.2, 0.4] so the model self-heals on
                     low-confidence segments instead of silently dying.
  • Silence gate   : CLIENT_SILENCE_MUL 2.5→1.5  (Hindi callers are often
                     softer-spoken; 2.5 was gating real speech as silence).

HALLUCINATION / CONFIDENCE FILTERS
  • _is_hallucination : Hinglish single-char tokens like "h", "n" no longer
                        trigger the short-phrase uniqueness filter.
  • _is_low_confidence: Hinglish filler ratio threshold 0.7→0.85 to stop
                        valid affirmatives ("haan ji", "aur kya") being dropped.
  • MIN_DEDUP_OVERLAP : kept at 3 — correct behaviour confirmed for both modes.
  • Length gate       : min char threshold 5→3 for Hinglish (short responses
                        like "ok", "ha" are valid acknowledgements).

ENGLISH
  • No changes — 95% accuracy is already production-acceptable.

GENERAL
  • Typed constants, docstrings, and inline rationale comments added for
    Codex review readability.
  • Worker thread exception handling hardened (catches broad Exception and
    logs rather than silently killing the worker).
  • Audio normalisation: per-chunk peak-norm only when max > 0 (was already
    guarded; kept).
  • save_full_audio: writes at SAMPLE_RATE (16 kHz) for both streams so
    saved files are directly re-playable without resampling.
"""
